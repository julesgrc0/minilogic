...
class ExpressionOptimizer {
  ...
  public optimize(): Statement[] {
    ...
    for (const stmt of this.ast) {
      if (stmt.type === StatementType.Assignment || stmt.type === StatementType.FunctionDefinition) {
  ...
  private removeDeadCode(expr: Expression): Expression {
    if (expr.type === ExpressionType.BinaryExpression) {
      ...
      if (expr.operator === Operators.Or) {
        if (left.type === ExpressionType.Number && left.value === 1) return left;
        if (right.type === ExpressionType.Number && right.value === 1) return right; 
        if (JSON.stringify(left) === JSON.stringify(right)) return left; 
      }
      
      if (expr.operator === Operators.And) {
        if (left.type === ExpressionType.Number && left.value === 0) return left; 
        if (right.type === ExpressionType.Number && right.value === 0) return right; 
        if (JSON.stringify(left) === JSON.stringify(right)) return left;
      }
    ...
    if (expr.type === ExpressionType.UnaryExpression && expr.operator === Operators.Not) {
      const subExpr = this.removeDeadCode(expr.operand);
  
      if (subExpr.type === ExpressionType.UnaryExpression && subExpr.operator === Operators.Not) {
  ...
  private regroupExpressions(expr: Expression): Expression {
    if (expr.type === ExpressionType.UnaryExpression && expr.operator === Operators.Not) {
      ...
      if (subExpr.type === ExpressionType.UnaryExpression && subExpr.operator === Operators.Not) {
        ...
        const regroupeCases: Omit<Record<Operators, Operators>, Operators.Not> = {
        ...
        if (subExpr.operator in regroupeCases) {
          return {
            ...
            operator: regroupeCases[subExpr.operator as keyof typeof regroupeCases],
