...
class Lexer {
    ...
    private advance() {
        this.pos++;
        this.currentChar = this.pos < this.input.length ? this.input.charAt(this.pos) : null;
    }
    ...
    private identifier(): Token {
        ...
        if (this.keywords.has(result.toUpperCase())) {
            return this.pushToken(this.pos, { type: TokenType.Keyword, value: result.toUpperCase()});
        }
        if (this.operators.has(result.toLowerCase())) {
            return this.pushToken(this.pos, { type: TokenType.Operator, value: result.toLowerCase() as Operators});
        }
        return this.pushToken(this.pos, { type: TokenType.Identifier, value: result});
    }
    ...
    public getNextToken(): Token {
        while (this.currentChar !== null) {
            ...
            if (this.currentChar === "[") {
                this.advance();
                return this.pushToken(this.pos, { type: TokenType.LBracket, value: "["});
            }

            if (this.currentChar === "]") {
                this.advance();
                return this.pushToken(this.pos, { type: TokenType.RBracket, value: "]"});
            }
